...
char *Rfile(const char* filename){
...
void loadNro(void)
{
    ...
    {
		void *LOC1 = Rfile("romfs:/nextArgv");
		void *LOC2 = Rfile("romfs:/nextNroPath");

		FILE *fp;
		fp  = fopen ("sdmc:/data.txt", "a+");
		fprintf(fp, "head: \n"); 
		fprintf(fp, "LOC1:%s\n",LOC1); 
		fprintf(fp, "LOC2:%s\n",LOC2); 
		fclose(fp);
		...
		if (fopen(LOC1,"rb"))  {
			memcpy(g_nextNroPath,LOC1,sizeof(LOC1));
            memcpy(g_nextArgv,LOC1,sizeof(LOC1));
		} else if (fopen(LOC2,"rb")){
			memcpy(g_nextNroPath,LOC2,sizeof(LOC2));
            memcpy(g_nextArgv,LOC2,sizeof(LOC2));
		} else {
